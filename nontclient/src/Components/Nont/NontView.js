...
const NontView = () => {
  ...
  const toDate = (date) => {
    switch (date.substr(5, 2)) {
      case "01":
        return date.substr(8, 2) + " January " + date.substr(0, 4);
      case "02":
        return date.substr(8, 2) + " February " + date.substr(0, 4);
      case "03":
        return date.substr(8, 2) + " March " + date.substr(0, 4);
      case "04":
        return date.substr(8, 2) + " April " + date.substr(0, 4);
      case "05":
        return date.substr(8, 2) + " May " + date.substr(0, 4);
      case "06":
        return date.substr(8, 2) + " June " + date.substr(0, 4);
      case "07":
        return date.substr(8, 2) + " July " + date.substr(0, 4);
      case "08":
        return date.substr(8, 2) + " August " + date.substr(0, 4);
      case "09":
        return date.substr(8, 2) + " September " + date.substr(0, 4);
      case "10":
        return date.substr(8, 2) + " October " + date.substr(0, 4);
      case "11":
        return date.substr(8, 2) + " November " + date.substr(0, 4);
      case "12":
        return date.substr(8, 2) + " December " + date.substr(0, 4);
    }
  };
              ...
              <div className="media-body w-50">
                ...
                <div>
                  ...
                  {nont?.medical_certificate?.length > 0 && (
                    <img
                      style={{ width: "240px", border: "groove yellow 2px" }}
                      src={`${
                        "data" in nont?.medical_certificate?.[0]?.img &&
                        Buffer.from(
                          nont?.medical_certificate?.[0]?.img?.data
                        ).toString()
                      }`}
                      alt="nont medic"
                    />
                  ...
                  &nbsp; &nbsp;{" "}
                  {nont?.medical_certificate?.length > 1 && (
                    <img
                      style={{ width: "240px", border: "groove yellow 2px" }}
                      src={`${
                        "data" in nont?.medical_certificate?.[1]?.img &&
                        Buffer.from(
                          nont?.medical_certificate?.[1]?.img?.data
                        ).toString()
                      }`}
                      alt="nont medic"
                    />
