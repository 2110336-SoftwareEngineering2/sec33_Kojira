...
updateAccount = async (req, res) => {
  try {
    const data = req.body;
    if (data.email) {
      const emailFindResult = await this.NontSitter.findOne({ email: data.email });
      if (emailFindResult)
        return res.status(403).send("Email already exists.");
    }
    if (data.name) {
      const nameFindResult = await this.NontSitter.findOne({ name: data.name });
      if (nameFindResult)
        return res.status(403).send("Username already exists.");
    }
    await this.NontSitter.findByIdAndUpdate(
      data._id,
...
create = async (req, res) => {
  ...
  try {
    ...
    const nameFindResult = await this.NontSitter.findOne({ name: req.body.name });
    if (nameFindResult)
      return res.status(403).send("Username already exists.");
  } catch (error) {
    return res
  ...
  try {
    ...
    try {
      const nontSitterAccount = await this.NontSitter.create(newBody);
      return res.send(this._.pick(nontSitterAccount, ["_id", "email", "name"]));
    } catch (error) {
      throw error;
    }
  } catch (error) {
...
login = async (req, res) => this.LoginController.login(req, res, this.NontSitter)
...
adminUpdateNontSitter = async (req, res) => {
  try {
    ...
    const updatedNontSitter = await this.NontSitter.updateOne(newQuery, newBody);
